...
Full pipeline for adding `<ptcgDataSetId>` / `<code>` (find `ptcgDataSetId` from pokemon-tcg-data's `sets/en.json`, its `id` field — see CLAUDE.md):

1. `node scripts/fetch-set.mjs <ptcgDataSetId> <code>` — or, if this set is a Trainer Gallery/Shiny Vault/Galarian Gallery-style subset that pokemon-tcg-data splits out as its own id but Limitless files under its base set's page, `node scripts/fetch-set.mjs <ptcgDataSetId> <code> <baseLimitlessCode>` (see CLAUDE.md "Subsets that share their base set's Limitless page"). Check whether a given subset actually needs the third argument — try without it first; a 404 on the first card means it does. If the set is additionally a throwback-reprint subset whose `number` field collides within the set (check first — don't assume), also pass a fourth `<sequentialPrefix>` argument (see CLAUDE.md "Reprint subsets with non-sequential, non-unique numbers").
